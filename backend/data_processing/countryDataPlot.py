...
from data_processing.constants import COLORS, mpl_config, column_name
...
def compare_country_data(
    ...
    if not (isinstance(countries, list) or isinstance(countries, tuple)):
        raise ValueError("Please pass in a list/tuple of country/countries")


    # if len(countries) > 1, only one thing can be plotted

    country_dictionary = get_country_dictionary(countries, plot)

    fig.patch.set_facecolor([0,0,0,0])

    for index, cont in enumerate(country_dictionary.keys()):
        # --------color config----------------------------
        ...
        if plot_type == 'line':
            all_axes[index].plot(
                country_dictionary[cont].index,
                country_dictionary[cont][column_name[plot]],
                color = COLORS[index]
            )
    ...
    backend_dir = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
    os.makedirs(os.path.join(backend_dir, 'temp'), exist_ok=True)
    filepath = os.path.join(backend_dir, 'temp', f"{countries}.png")
    fig.savefig(filepath)
